...
class UnpublishRequest : BaseRequestJson<CommonOperationResult<string>>
{
    ...
    protected override string RelationalUri => $"/api/m1/file/unpublish?access_token={_auth.AccessToken}";

    protected override byte[] CreateHttpContent()
    {
        var data = $"weblink={Uri.EscapeDataString(_publicLink)}&email={_auth.Login}&x-email={_auth.Login}";
