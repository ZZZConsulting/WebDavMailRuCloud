...
1. Добавлена поддержка TOTP кодов для авторизации в Яндекс.Диск
...
Схема применения Эмулятора при использовании NAS, на примере NAS Synology:
...
3. Создать **Проект** в **Container Manager** с созданными каталогом в качестве _Пути_,
   в качестве _Источника_ указать _Создать docker-compose.yml_ со следующим содержимым:
...
Способ подтверждения входа в **Яндекс.Диск** должен быть _Пароль+Одноразовые_коды(Яндекс.Ключ)_.
_Секретный_код_TOTP_ можно получить, если во время перехода на одноразовые пароли TOTP выбрать ручной ввод.
